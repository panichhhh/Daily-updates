...
ENERGY_DEV_SEARCHES = [
    "OPEC+ output OR production quota OR Saudi Arabia oil policy",
    "Russia oil sanctions OR Iran oil OR energy sanctions OR export ban",
    "LNG OR natural gas Europe OR gas prices OR pipeline OR supply",
    "energy transition OR renewables OR solar OR wind OR EV demand OR grid",
    "nuclear power OR electricity demand OR IEA OR EIA OR energy outlook",
]
THAI_FEEDS = [
...
def collect_energy_dev_news(cap_total=6):
    seen, items = set(), []
    for q in ENERGY_DEV_SEARCHES:
        try:
            _add(seen, items, fetch_entries(gnews_url(q)), cap=4)
        except Exception as e:
            print(f"news: failed energy-dev '{q}': {e}", file=sys.stderr)
    return items[:cap_total]
...
def build_data(equities, commod, energy, rates, movers, world, energy_news,
    ...
    blocks = [
        ...
        "GLOBAL ENERGY DEVELOPMENTS (OPEC+, sanctions, LNG/gas, transition, power):\n"
        + news_text(energy_dev),
...
7) "Global energy developments" - the bigger-picture energy news below: OPEC+ policy, sanctions and supply disruptions, natural gas/LNG, the energy transition (renewables, EVs, grid), and power/nuclear. Summarize 3-5 items in 1-2 sentences each with any figures, and note the implication for oil/gas prices.
8) "Global & macro news" - 4-6 other important items, each 1-2 sentences with the key figures.
9) "Thai market" - the SET Index and USD/THB figures with brief context, then summarize 4-6 of the Thai news items below (from Kaohoon, Bangkok Post, Prachachat, etc.) in 1-2 detailed sentences each, naming the stocks/sectors involved.
10) "Watch today/tonight" - a concise bullet list of the key events, data releases, earnings and price levels investors should watch next.
11) One-line sign-off.
...
def format_brief_plain(equities, commod, energy, rates, movers, world,
                       energy_news, energy_dev, thai_mkt, thai_news, date_str):
    ...
    lines += ["🌍 Global energy developments", news_text(energy_dev), ""]
...
def main():
    ...
    energy_dev = collect_energy_dev_news()
        ...
        try:
            brief = write_brief_with_claude(
                build_data(equities, commod, energy, rates, movers, world,
                           energy_news, energy_dev, thai_mkt, thai_news),
                date_str,
            )
    ...
    if not brief:
        brief = format_brief_plain(
            ...
            energy_dev, thai_mkt, thai_news, date_str
        )
